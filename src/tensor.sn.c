...
{
    if (g_record_mode) {
        /* Build differentiable batchnorm in ggml graph.
         * Mean and inv-std are computed on host and entered as graph constants.
         * Gradients flow through (x - mean) * inv_std * weight + bias. */
        TPool *pt = unwrap(t);
        ...
        /* Compute batch mean/var on host */
        float *bmean = (float *)calloc((size_t)feat_dim, sizeof(float));
        float *bvar  = (float *)calloc((size_t)feat_dim, sizeof(float));
        for (int64_t r = 0; r < num_nodes; r++)
            for (int64_t c = 0; c < feat_dim; c++)
                bmean[c] += pt->data[r * feat_dim + c];
        for (int64_t c = 0; c < feat_dim; c++)
            bmean[c] /= (float)num_nodes;
        for (int64_t r = 0; r < num_nodes; r++)
            for (int64_t c = 0; c < feat_dim; c++) {
                float d = pt->data[r * feat_dim + c] - bmean[c];
                bvar[c] += d * d;
            }
        for (int64_t c = 0; c < feat_dim; c++)
            bvar[c] /= (float)num_nodes;

        /* Update running stats for inference (momentum = 0.1) */
        if (training) {
            TPool *pm = unwrap(running_mean);
            TPool *pv = unwrap(running_var);
            for (int64_t c = 0; c < feat_dim && c < pm->n_elem; c++) {
                pm->data[c] = 0.9f * pm->data[c] + 0.1f * bmean[c];
                pv->data[c] = 0.9f * pv->data[c] + 0.1f * bvar[c];
            }
        }

        /* Mean constant tensor */
        int mean_idx = pool_alloc(feat_dim, 1, 2);
        memcpy(g_pool[mean_idx].data, bmean, (size_t)feat_dim * sizeof(float));
        struct ggml_tensor *gmean = ggml_new_tensor_2d(g_param_ctx, GGML_TYPE_F32, feat_dim, 1);
        ggml_set_name(gmean, "bn_mean");
        ggml_set_input(gmean);
        g_record_map[mean_idx] = gmean;

        /* Inv-std constant tensor: 1/sqrt(var + eps) */
        int istd_idx = pool_alloc(feat_dim, 1, 2);
        for (int64_t c = 0; c < feat_dim; c++)
            g_pool[istd_idx].data[c] = 1.0f / sqrtf(bvar[c] + bn_eps);
        struct ggml_tensor *gistd = ggml_new_tensor_2d(g_param_ctx, GGML_TYPE_F32, feat_dim, 1);
        ggml_set_name(gistd, "bn_istd");
        ggml_set_input(gistd);
        g_record_map[istd_idx] = gistd;

        free(bmean);
        free(bvar);

        /* centered = x - repeat(mean) */
        struct ggml_tensor *mean_rep = ggml_repeat(ctx, gmean, gt);
        struct ggml_tensor *centered = ggml_sub(ctx, gt, mean_rep);

        /* normalized = centered * repeat(inv_std) */
        struct ggml_tensor *istd_rep = ggml_repeat(ctx, gistd, gt);
        struct ggml_tensor *normed   = ggml_mul(ctx, centered, istd_rep);

        /* output = weight * normalized + bias */
        struct ggml_tensor *w_rep  = ggml_repeat(ctx, gw, gt);
        struct ggml_tensor *b_rep  = ggml_repeat(ctx, gb, gt);
        struct ggml_tensor *scaled = ggml_mul(ctx, w_rep, normed);
        struct ggml_tensor *output = ggml_add(ctx, scaled, b_rep);
